...
//							}//END if 1==0
				...
				if (Grass_painting & Enable_real_time_paint) {
					if (Mouse.current.rightButton.wasPressedThisFrame & !Keyboard.current.leftShiftKey.wasPressedThisFrame
						...
						if(Tag_based_player){
							if(player != null){
								...
								if(playerCams != null && playerCams.Length > 0 && playerCams[0] != null){
									ray = playerCams[0].ScreenPointToRay (Mouse.current.position.ReadValue());
									found_cam = true;
								}else{
									if(Camera.main != null){
							...
							if(Camera.main != null){
								...
								ray = Camera.current.ScreenPointToRay (Mouse.current.position.ReadValue());
								found_cam = true;
							}
						}
				...
				if (Fence_painting & Enable_real_time_paint
					...
					if (Input.GetKeyDown (KeyCode.LeftShift)) {
						...
						if (Mouse.current.rightButton.wasPressedThisFrame && Fence_poles_tmp.Count > 0) {
