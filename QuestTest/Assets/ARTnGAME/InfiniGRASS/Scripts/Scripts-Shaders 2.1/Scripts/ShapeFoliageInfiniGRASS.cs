...
namespace Artngame.INfiniDy {
    ...
    public class ShapeFoliageInfiniGRASS : MonoBehaviour
    {
                ...
                if (Camera.main != null)
                {
                    ...
                    if (Physics.Raycast(ray, out hit, Mathf.Infinity))
                    {
                        ...
                        if (TextureInteract)
                        {
                            ...
                            if (inBounds)
                            { //v2.0.9
                              //							if (TextureErase && Mouse.current.rightButton.isPressed ) { //TexCombBend
                              //								for (int i = 0; i < pixels.Length; i++) {
                              //									pixels [i] = Color.white * EraseDepth;
                              //								}
                              //							}
                                if (TextureComb && Mouse.current.rightButton.isPressed)
                                {
                                ...
                                if (TextureErase && Mouse.current.rightButton.isPressed)
                                { //TexCombBend
                                ...
                                if ((TextureErase || TextureComb || TexturePaintAlpha) && Mouse.current.rightButton.isPressed)
                                { //v2.1.12
        ...
        void LateUpdate()
        {
            ...
            //v2.1.28
            if (runtimePlant && Mouse.current.rightButton.isPressed)
            {
                Ray ray = Camera.main.ScreenPointToRay(Mouse.current.position.ReadValue());
                RaycastHit hit = new RaycastHit();
                if (Physics.Raycast(ray, out hit, Mathf.Infinity))
            ...
            {//v1.1	
                ...
                else if (globalManager.TextureInteract)
                {
                    ...
                    {   //TextureInteract) {
                        ...
                        {// && Mouse.current.rightButton.isPressed ) { //TexCombBend
                         //v2.1.16
